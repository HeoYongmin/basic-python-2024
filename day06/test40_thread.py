...
class BackWorker(QThread): # pyQt에서 스레드 클래스 상속
    ...
    def run(self) -> None: # 스레드 실행할 때는 항상 run 사용
        ...
        ###self.parent.pgbTask.setValue(0) # 프로그레스바 0부터 시작 !!QThread에션 UI 관련된 처리를 할 수 없음
...
class qtwin_exam(QWidget): # UI 스레드
    ...
    def btnStartClicked(self):
        ...
        th.setLog.connect(self.setTxbLog) # TextBrowser 위젯에 진행 상황 출력
    ...
    @pyqtSlot(str)  # BackWorker 스레드에서 self.setLog.emit()이 동작해서 실행
    def setTxbLog(self, msg):
        self.txbLog.append(msg)

    @pyqtSlot(int) # BackWorker 스레드에서 self.setSignal.emit()이 동작해서 실행
